...
- 문서와 코드를 함께 관리합니다.
- 기획서, 로드맵, 결정 기록, 작업 로그는 프로젝트 저장소의 `docs/` 아래 Markdown 파일로 둡니다.
- Slack은 요청과 응답을 중계하고, 장기 작업 맥락은 Git 이력으로 추적합니다.
...
- `inbox/<project_id>/*.md`: Slack 요청을 사람이 읽기 쉬운 작업 파일로 저장합니다.
